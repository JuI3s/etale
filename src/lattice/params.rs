...
pub fn find_primitive_2d_root(q: u64, d: usize) -> Option<u64> {
    ...
    (2..q).find_map(|g| {
        ...
        (pow_mod(psi, two_d, q) == 1 && pow_mod(psi, d as u64, q) == q - 1).then_some(psi)
    })
}
...
pub const NTT_FRIENDLY: &[&RingParams] = &[&DILITHIUM_2, &FALCON_512];
...
mod tests {
    ...
    fn test_find_primitive_root() {
        ...
        let psi_dil = DILITHIUM_2.find_primitive_root().unwrap();
        assert!(DILITHIUM_2.verify_primitive_root(psi_dil));
        assert!(DILITHIUM_2.verify_primitive_root(1753));

        let psi_fal = FALCON_512.find_primitive_root().unwrap();
        assert!(FALCON_512.verify_primitive_root(psi_fal));
    ...
    fn test_compressed_constructor() {
        ...
        assert_eq!(c4.beta_inf, 60);

        let c16 = RingParams::compressed(&HACHI, 16, "test_k16");
        assert_eq!(c16.d, 64);
        assert_eq!(c16.beta_inf, 240);
    }
